/*
  ...
  1. New Tables
    - `oms_tailors` - Tailor profiles and credentials
    - `oms_tailor_performance` - Performance metrics for tailors
    - `oms_sales_performance` - Performance metrics for sales staff
    - `oms_order_assignments` - Order assignment tracking
    - `oms_quality_checks` - Quality check records

  2. Enhanced Tables
    - Update `oms_orders` to include assignment and workflow fields
    - Update `oms_users` role enum to include 'tailor'
...
CREATE POLICY "Admins can manage tailors"
  ...
  USING (
    EXISTS (
      ...
      WHERE oms_users.id::text = uid()::text 
...
CREATE POLICY "Tailors can read own performance"
  ...
  USING (
    EXISTS (
      ...
      AND oms_tailors.user_id::text = uid()::text
    )
  );
...
CREATE POLICY "Sales staff can read own performance"
  ...
  FOR SELECT
  TO authenticated
  USING (sales_staff_id::text = uid()::text);
